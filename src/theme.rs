...
impl Theme {
    pub fn with_yaml(yaml: &str) -> Result<IconTheme, Box<dyn std::error::Error>> {
        let theme: Theme = serde_yaml::from_str(yaml).map_err(|e| Box::new(e) as Box<dyn std::error::Error>)?;
        Ok(theme.icon_theme)
    }

    pub fn from_path(path: &str) -> Result<IconTheme, Box<dyn std::error::Error>> {
        let content = match std::fs::read_to_string(path) {
            Ok(c) => c,
            Err(e) => return Err(Box::new(e)),
        };
        Self::with_yaml(&content)
    }
}
